...
class WhisperXBenchmark:
    """WhisperX Benchmark - Optimized transcription model with microphone or audio file input"""
    
    def __init__(self, use_gpu: bool = False, model_size: str = "base", language: Optional[str] = None):
    ...
    def _load_model(self):
        ...
        if self.use_gpu:
            ...
            for compute_type in compute_candidates:
                try:
                    logger.info(f"Trying whisperx compute_type={compute_type} on GPU...")
                    ...
                    logger.info(f"Loaded whisperx with compute_type={compute_type}")
                    break
...
def main():
    parser = argparse.ArgumentParser(
                description='WhisperX Benchmark - Optimized transcription with microphone or audio file input',
        epilog="""
Examples:
  ...
  python whisperx_benchmark.py --duration 10 --output resultados/whisperx_result.json
    ...
    parser.add_argument('--show-transcription-early', action='store_true', help='Print full transcription before chunk metrics finish')
