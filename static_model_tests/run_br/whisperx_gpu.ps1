...
$scriptPath = Join-Path $projectRoot 'whisperx_benchmark.py'
...
$jobs = foreach ($duration in $durations) {
    foreach ($modelSize in $modelSizes) {
        @{
            ...
            OutputFile = Join-Path $projectRoot "resultados/whisperx/gpu/whisperx-$modelSize-$($duration.Label)-br-gpu.json"
        }
    }
}
...
foreach ($job in $jobs) {
    ...
    Write-Host ''
    Write-Host ('=' * 80)
    Write-Host "Rodando whisperx GPU | duration=$($job.DurationLabel) | model-size=$($job.ModelSize)"
