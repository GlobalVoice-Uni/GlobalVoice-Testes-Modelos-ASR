...
$jobs = foreach ($duration in $durations) {
    foreach ($modelSize in $modelSizes) {
        @{
            ...
            OutputFile = Join-Path $projectRoot "resultados/faster-whisper/gpu/fw-$modelSize-$($duration.Label)-br-gpu.json"
        }
    }
}
...
foreach ($job in $jobs) {
    Write-Host ''
    Write-Host ('=' * 80)
    Write-Host "Rodando faster-whisper GPU | duration=$($job.DurationLabel) | model-size=$($job.ModelSize)"
