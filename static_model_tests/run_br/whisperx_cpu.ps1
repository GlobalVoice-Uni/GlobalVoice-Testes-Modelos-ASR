...
$jobs = foreach ($duration in $durations) {
    foreach ($modelSize in $modelSizes) {
        @{
            ...
            OutputFile = Join-Path $projectRoot "resultados/whisperx/cpu/whisperx-$modelSize-$($duration.Label)-br-cpu.json"
        }
    }
}
...
foreach ($job in $jobs) {
    ...
    Write-Host ''
    Write-Host ('=' * 80)
    Write-Host "Rodando whisperx CPU | duration=$($job.DurationLabel) | model-size=$($job.ModelSize)"
