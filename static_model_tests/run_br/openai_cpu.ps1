...
$scriptPath = Join-Path $projectRoot 'whisper_benchmark_complete.py'
...
$durations = @(
    @{
        Label = '10s'
        InputFile = Join-Path $projectRoot 'audios/10sBR.mp4'
        ReferenceFile = Join-Path $projectRoot 'textos/10sBR.txt'
    }
    @{
        Label = '30s'
        InputFile = Join-Path $projectRoot 'audios/30sBR.mp4'
        ReferenceFile = Join-Path $projectRoot 'textos/30sBR.txt'
    }
    @{
        Label = '60s'
        InputFile = Join-Path $projectRoot 'audios/60sBR.mp4'
        ReferenceFile = Join-Path $projectRoot 'textos/60sBR.txt'
    }
)
...
$jobs = foreach ($duration in $durations) {
    foreach ($modelSize in $modelSizes) {
        @{
            ...
            OutputFile = Join-Path $projectRoot "resultados/openai-whisper/cpu/openai-$modelSize-$($duration.Label)-br-cpu.json"
        }
    }
}
...
foreach ($job in $jobs) {
    ...
    Write-Host ''
    Write-Host ('=' * 80)
    Write-Host "Rodando openai-whisper CPU | duration=$($job.DurationLabel) | model-size=$($job.ModelSize)"
