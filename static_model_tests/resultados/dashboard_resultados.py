...
def build_filtered_df(df: pd.DataFrame) -> pd.DataFrame:
    with st.sidebar:
        ...
        language_options = sorted(df["Idioma"].dropna().unique().tolist())
        ...
        lang_filtered = df[df["Idioma"].isin(selected_languages)].copy()

        hardware_options = sorted(lang_filtered["Hardware"].dropna().unique().tolist())
        size_options = _ordered_sizes(lang_filtered["Tamanho"].dropna().unique().tolist())
        interval_options = sorted(lang_filtered["Intervalo_s"].dropna().astype(int).unique().tolist())
        model_options = _ordered_models(lang_filtered["Modelo"].dropna().unique().tolist())
    ...
    filtered = lang_filtered[
        lang_filtered["Hardware"].isin(selected_hardware)
        & lang_filtered["Tamanho"].isin(selected_sizes)
        & lang_filtered["Intervalo_s"].isin(selected_intervals)
        & lang_filtered["Modelo"].isin(selected_models)
    ].copy()
...
def metric_cards(filtered: pd.DataFrame) -> None:
    ...
    col1.metric("Registros", int(len(filtered)))
    col2.metric("Idiomas", int(filtered["Idioma"].nunique()))
    col3.metric("Modelos", int(filtered["Modelo"].nunique()))
    col4.metric("Hardwares", int(filtered["Hardware"].nunique()))
    col5.metric("Tamanhos", int(filtered["Tamanho"].nunique()))
...
def main() -> None:
    ...
    st.caption(
        "Fonte: resultados_gerais.xlsx | suporta EN e BR | filtráveis na barra lateral"
    )
