...
$scriptPath = 'd:/CODE/PROJETO - GLOBALVOICE/GlobalVoice-Testes/whisperx_benchmark.py'
...
$jobs = foreach ($duration in $durations) {
    foreach ($modelSize in $modelSizes) {
        @{
            ...
            OutputFile = Join-Path $projectRoot "resultados/whisperx/gpu/whisperx-$modelSize-$($duration.Label)-en-gpu.json"
        }
    }
}
